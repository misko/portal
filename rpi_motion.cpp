                 ...
                 Scalar & color)
{
	...
	fprintf(stderr,"NOT ENOGUH MOTION\n");
    }
    return 0;
}
...
{
    const string DIR = "./pics/"; // directory where the images will be stored
    ...
    Camera.grab(); Camera.retrieve (prev_frame);
    Camera.grab(); Camera.retrieve (current_frame);
    Camera.grab(); Camera.retrieve (next_frame);
	...
	fprintf(stderr,"%d %d \n", number_of_changes, number_of_sequence);        
